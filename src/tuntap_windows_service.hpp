...
//			: boost::asio::detail::service_base<tuntap_windows_service>(io_context)
			...
			//, m_handle(INVALID_HANDLE_VALUE)
		{
			...
			for (int i = 0; ; i++)
			{
				...
				else
				{
					...
					if (!_tcscmp(component_id, TEXT("tap0901")))
					{
						...
						std::string tmp;
#ifdef UNICODE
						details::utf16_utf8(net_cfg_instance_id, tmp);
#else
						tmp = net_cfg_instance_id;
#endif
						dev_map.insert(std::make_pair(tmp, ""));

						INFO("component_id:%s, net_cfg_instance_id:%s ", component_id, tmp.c_str());
					}
				}
			}
			...
			for (int i = 0; ; i++)
			{
				...
				else
				{
					...
					{
#ifdef UNICODE
						details::utf16_utf8(name_data, dev_name);
#else
						dev_name = name_data;
#endif
					}
					...
					{
#ifdef UNICODE
						details::utf16_utf8(enum_name, guid_key);
#else
						guid_key = enum_name;
#endif
					}
					...
					if (iter != dev_map.end())
					{
						...
						INFO("Tuntap device name:%s ", dev_name.c_str());
					}
				}
			}
		...
		bool open(const dev_config& cfg)
		{
			...
			//INFO("%s.%s,%s", USERMODEDEVICEDIR , cfg.guid_.c_str() , TAPSUFFIX);
			...
			if (handle == INVALID_HANDLE_VALUE) {
				ERROR2( "CreateFile error.");
				return false;
			}
			INFO("tun handle:%d", handle);
			...
			{
				ERROR2("WARNING:  Tap-Win32 driver version %d . does not support IPv6 in TUN mode. IPv6 will not work. Upgrade your Tap-Win32 driver.", (int)version.major);
			}
			...
			if (cfg.dev_type_ == tuntap_service::dev_tun
				&& version.major == 9 && version.minor == 8)
			{
				ERROR2( "ERROR:  Tap-Win32 driver version %d.%d is buggy regarding small IPv4 packets in TUN mode. Upgrade your Tap-Win32 driver.",(int)version.major ,(int)version.minor);
			}
			...
			if (cfg.dev_type_ == tuntap_service::dev_tun)
			{
				...
				{
					uint32_t ep1 = ntohl(tun_addrs[1]);
					uint32_t ep0 = ntohl(tun_addrs[0]);
					uint32_t ep2 = ntohl(tun_addrs[2]);
					INFO("Set TAP-Windows TUN subnet mode network/local/netmask = %x/%x/%x" ,ep1 , ep0 ,ep2);
				}
			}
			else
			{
...
// 					cfg.dev_name_);
			...
			ep[0] = tun_addrs[0]; //htonl(tt->local);
			ep[1] = tun_addrs[2]; //htonl(tt->adapter_netmask);
			uint32_t dhcp_ip;
			inet_pton(AF_INET, "126.24.255.254", &dhcp_ip);
			ep[2] = dhcp_ip;


			/* lease time in seconds */
			ep[3] = (uint32_t)3600 * 24;

			if (!DeviceIoControl(handle, TAP_IOCTL_CONFIG_DHCP_MASQ,
				ep, sizeof(ep),
				ep, sizeof(ep), &len, NULL))
				ERROR2("ERROR: The TAP-Win32 driver rejected a DeviceIoControl call to set TAP_IOCTL_CONFIG_DHCP_MASQ mode");

			INFO("Notified TAP-Win32 driver to set a DHCP IP/netmask of %x/%x on interface [DHCP-serv: %x, lease-time: %d]",
				ep[0],
				ep[1],
				ep[2],
				ep[3]			);
			...
			{
				INFO("TAP-Windows MTU=%d" , (int)mtu);
			}
		...
		*/
        ...
        int get_handle(uint64_t& handle){
            handle = (uint64_t)m_handle;
            return 0;
        }
